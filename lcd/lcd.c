...
void display_char(char c) {
    ...
    PGM_P fdata; 
    uint8_t bits, mask;
    uint8_t char_space = 9;
    uint8_t line_space = 8;
    uint16_t char_size_x = 7;
    uint16_t char_size_y = 10;
    uint16_t char_start=display.x, char_end=display.x + char_size_x,
             char_top_start=display.y, char_top_end=display.y + char_size_y;

    /*   New line starts a new line, or if the end of the
         display has been reached, clears the display.
    */
    if (c == '\n') { 
        display.x=0; display.y+=line_space;
        if (display.y >= display.height) { clear_screen(); }
        return;
    }
    ...
    fdata = (c - ' ')*8 + font5x7;
    write_cmd(PAGE_ADDRESS_SET);
    write_data16(char_top_start);
    write_data16(char_top_end);
    for(x=char_start; x<=char_end; x++) {
        write_cmd(COLUMN_ADDRESS_SET);
        write_data16(x);
        write_data16(x);
        write_cmd(MEMORY_WRITE);
        bits = pgm_read_byte(fdata++);
        for(y=char_top_start, mask=0x01; y<=char_top_end; y++, mask<<=1)
            write_data16((bits & mask) ? display.foreground : display.background);
    }
    write_cmd(COLUMN_ADDRESS_SET);
    write_data16(x);
    write_data16(x);
    write_cmd(MEMORY_WRITE);
    for(y=char_top_start; y<=char_top_end; y++)
        write_data16(display.background);

    display.x += char_space;
    if (display.x >= display.width) { display.x=0; display.y+=line_space; }
}
...
void small_display_string_xy(char *str, uint16_t x, uint16_t y) {
    uint8_t i;
    display.x = x;
    display.y = y;
    for(i=0; str[i]; i++)
        old_display_char(str[i]);
